...
import emi.dev.emi.emi.config.SidebarSide;
...
import emi.dev.emi.emi.runtime.EmiFavorite;
import emi.dev.emi.emi.screen.widget.ResolutionButtonWidget;
...
public class RecipeScreen extends REMIScreen implements EmiScreen {
	...
	public RecipeScreen(GuiContainer old, Map<EmiRecipeCategory, List<EmiRecipe>> recipes) {
		...
		this.old = old;
		arrows = List.of(new SizedButtonWidget(x + 2, y - 18, 12, 12, 0, 0, () -> tabs.size() > tabPageSize, w -> setPage(tabPage - 1, tab, page)),
	...
	public void render(DrawContext raw, int mouseX, int mouseY, float delta) {
		...
		if (client.fontRenderer.getStringWidth(text.asString()) > minimumWidth - 40) {
			int extraWidth = client.fontRenderer.getStringWidth("...");
		...
		if (categoryHovered) {
			context.raw().drawTooltip(client.fontRenderer, List.of(tab.category.getName(), EmiPort.translatable("emi.view_all_recipes")), mouseX, mouseY);
		}
		hoveredWidget = null;
		outer:
		for (WidgetGroup group : currentPage) {
