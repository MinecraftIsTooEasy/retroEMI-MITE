...
import emi.dev.emi.emi.api.stack.ListEmiIngredient;
import emi.dev.emi.emi.api.stack.TagEmiIngredient;
import emi.dev.emi.emi.config.EmiConfig;
import emi.dev.emi.emi.data.TagExclusions;
...
public class EmiTags {
	...
	public static <T> EmiIngredient getIngredient(Class<T> clazz, List<EmiStack> stacks, long amount) {
		...
		if (keys == null || keys.isEmpty()) {
			...
			return new ListEmiIngredient(List.of(map.values().stream().map(i -> i.copy().setAmount(1)).collect(Collectors.toList()),
